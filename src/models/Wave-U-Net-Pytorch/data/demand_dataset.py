        ...
        if not os.path.exists(self.hdf_dir):
            ...
            with h5py.File(self.hdf_dir, "w") as f:
                ...
                for idx, example in enumerate(tqdm(dataset[partition])):
                    ...
                    noisy_audio = noisy_audio[np.newaxis, :] if len(noisy_audio.shape) == 1 else noisy_audio
                    clean_audio = clean_audio[np.newaxis, :] if len(clean_audio.shape) == 1 else clean_audio
                    assert(clean_audio.shape[1] == noisy_audio.shape[1])
                    ...
                    grp.attrs["target_length"] = clean_audio.shape[1]
